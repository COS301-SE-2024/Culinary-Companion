...
const supURL =
  (Deno.env.get("_SUPABASE_URL") as string) || "http://localhost:54321";
...
async function getSuggestedFavorites(userId: string, corsHeaders: HeadersInit) {
  try {
    ...
    if (userFavoritesError) {
      throw new Error(
        `Error fetching user favorites: ${userFavoritesError.message}`
      );
    }
    ...
    const { data: similarUsersFavorites, error: similarUsersFavoritesError } =
      await supabase
        .from("userFavorites")
    ...
    if (similarUsersFavoritesError) {
      throw new Error(
        `Error fetching similar users' favorites: ${similarUsersFavoritesError.message}`
      );
    }

    //get other users favorites
    const otherUserIds = Array.from(
      new Set(similarUsersFavorites.map((fav) => fav.userid))
    );
    const { data: otherUsersRecipes, error: otherUsersRecipesError } =
      await supabase
        .from("userFavorites")
        .select("recipeid")
    ...
    if (otherUsersRecipesError) {
      throw new Error(
        `Error fetching other users' recipes: ${otherUsersRecipesError.message}`
      );
    }
...
async function getRecipeSuggestions(
  ...
  try {
    ...
    //spice level
    if (
      userPreferences.spiceLevel !== undefined &&
      userPreferences.spiceLevel !== null
    ) {
    ...
    //dietary constraints
    if (
      userPreferences.dietaryConstraints &&
      userPreferences.dietaryConstraints.length > 0
    ) {
...
async function addIngredientIfNotExists(
  ...
  if (!ingredientName || !measurementUnit) {
    console.error(
      "Validation Error: Ingredient name or measurement unit is missing."
    );
    return new Response(
      JSON.stringify({ error: "Invalid ingredient data provided" }),
      {
        status: 400,
        headers: corsHeaders,
      }
  ...
  try {
    ...
    if (existingIngredient && existingIngredient.length > 0) {
      return new Response(
        JSON.stringify({ ingredientId: existingIngredient[0].ingredientid }),
        {
          status: 200,
          headers: corsHeaders,
        }
    ...
    return new Response(
      JSON.stringify({ ingredientId: newIngredient[0].ingredientid }),
      {
        status: 200,
        headers: corsHeaders,
      }
...
async function searchRecipes(searchTerm: string, corsHeaders: HeadersInit) {
  ...
  try {
    const { data: recipes, error: recipesError } = await supabase
      ...
      .or(
        `name.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,course.ilike.%${searchTerm}%,cuisine.ilike.%${searchTerm}%,keywords.ilike.%${searchTerm}%`
      );
...
async function getAvailableIngredients(
  ...
  try {
    ...
    const { data: availableIngredients, error: availableIngredientsError } =
      await supabase
...
async function addRecipe(
  ...
  try {
    ...
    if (!recipeId) {
      ...
      return new Response(
        JSON.stringify({ error: "Failed to retrieve recipe ID" }),
        {
          status: 400,
          headers: corsHeaders,
        }
    ...
    for (const ingredient of ingredients) {
      const ingredientResponse = await addIngredientIfNotExists(
        ingredient.name,
        ingredient.unit,
        corsHeaders
      );
      ...
      if (ingredientResponse.status !== 200) {
        console.error(
          "Error adding or fetching ingredient:",
          await ingredientResponse.json()
        );
        return new Response(
          JSON.stringify({ error: "Failed to add or fetch ingredient" }),
          {
            status: 400,
            headers: corsHeaders,
          }
      ...
      if (recipeIngredientError) {
        ...
        return new Response(
          JSON.stringify({ error: recipeIngredientError.message }),
          {
            status: 400,
            headers: corsHeaders,
          }
    ...
    for (const appliance of appliances) {
      ...
      if (applianceError) {
        ...
        return new Response(
          JSON.stringify({ error: `Appliance not found: ${appliance.name}` }),
          {
            status: 400,
            headers: corsHeaders,
          }
      ...
      if (!applianceId) {
        ...
        return new Response(
          JSON.stringify({
            error: `Failed to retrieve appliance ID for ${appliance.name}`,
          }),
          {
            status: 400,
            headers: corsHeaders,
      ...
      if (recipeApplianceError) {
        console.error(
          "Error inserting recipe appliance:",
          recipeApplianceError
        );
        return new Response(
          JSON.stringify({ error: recipeApplianceError.message }),
          {
            status: 400,
            headers: corsHeaders,
          }
...
async function addToShoppingList(
  ...
  try {
    if (!userId || !ingredientName || quantity == null || !measurementUnit) {
      throw new Error(
        "User ID, ingredient name, quantity, and measurement unit are required"
      );
    }
    ...
    if (ingredientError) {
      return new Response(
        JSON.stringify({ error: `Ingredient not found: ${ingredientName}` }),
        {
          status: 400,
          headers: corsHeaders,
        }
    ...
    if (!ingredientId) {
      return new Response(
        JSON.stringify({
          error: `Failed to retrieve ingredient ID for ${ingredientName}`,
        }),
        {
          status: 400,
          headers: corsHeaders,
    ...
    if (shoppingItem) {
      ...
      if (shoppingListError) {
        return new Response(
          JSON.stringify({ error: shoppingListError.message }),
          {
            status: 400,
            headers: corsHeaders,
          }
...
async function editShoppingListItem(
  userId: string,
  ingredientName: string,
  quantity: number,
  measurementUnit: string,
...
async function addToPantryList(
  userId: string,
  ingredientName: string,
  quantity: number,
  measurementUnit: string
  ...
  try {
    ...
    if (pantryItem) {
      ...
      if (pantryListError) {
        return new Response(
          JSON.stringify({ error: pantryListError.message }),
          {
            status: 400,
            headers: corsHeaders,
          }
...
async function removeFromShoppingList(userId: string, ingredientName: string) {
  ...
  try {
    if (!userId || !ingredientName) {
      return new Response(
        JSON.stringify({ error: "User ID and Ingredient Name are required" }),
        {
          status: 400,
          headers: corsHeaders,
        }
...
async function getUserRecipes(userId: string, corsHeaders: HeadersInit) {
  ...
  try {
    ...
    if (!userRecipes || userRecipes.length === 0) {
      return new Response(
        JSON.stringify({ error: "No recipes found for this user" }),
        {
          status: 404,
          headers: corsHeaders,
        }
...
async function getUserFavourites(userId: string, corsHeaders: HeadersInit) {
  try {
    ...
    if (userFavouritesError) {
      throw new Error(
        `Error fetching user favourites: ${userFavouritesError.message}`
      );
    }

    if (!userFavourites || userFavourites.length === 0) {
      return new Response(
        JSON.stringify({ error: "No favourite recipes found for this user" }),
        {
          status: 404,
          headers: corsHeaders,
        }
    ...
    const { data: recipes, error: recipesError } = await supabase
      ...
      .select("*")
      .in("recipeid", recipeIds)
      .range(0, 20); // Fetch first 20 recipes
...
async function getRecipesBySpiceLevel(
  spiceLevel: number,
  corsHeaders: HeadersInit
) {
...
async function getIngredientsByCategory(
  ...
  try {
    ...
    if (ingredientsError) {
      console.error(
        "Error fetching ingredients by category:",
        ingredientsError
      );
...
async function getCategoryOfIngredient(
...
async function getIngredientNameAndCategory(corsHeaders: HeadersInit) {
  try {
    ...
    if (ingredientsError) {
      console.error(
        "Error fetching ingredients with categories:",
        ingredientsError
      );
      return new Response(JSON.stringify({ error: ingredientsError.message }), {
        status: 400,
...
async function getRecipe(recipeId: string, corsHeaders: HeadersInit) {
  try {
    ...
    if (appliancesError) {
      throw new Error(
        `Error fetching recipe appliances: ${appliancesError.message}`
      );
    }

    // Fetch appliance names based on appliance ids
    const applianceIds = appliancesData.map(
      (appliance) => appliance.applianceid
    );
    const { data: applianceNamesData, error: applianceNamesError } =
      await supabase
        .from("appliances")
        .select("name")
    ...
    if (applianceNamesError) {
      throw new Error(
        `Error fetching appliance names: ${applianceNamesError.message}`
      );
    }

    const applianceNames = applianceNamesData.map(
    ...
    if (ingredientsError) {
      throw new Error(
        `Error fetching recipe ingredients: ${ingredientsError.message}`
      );
    }

    // Fetch ingredient names and measurement units based on ingredient ids
    const ingredientIds = ingredientsData.map(
      (ingredient) => ingredient.ingredientid
    );
    const { data: ingredientNamesData, error: ingredientNamesError } =
      await supabase
        .from("ingredient")
    ...
    if (ingredientNamesError) {
      throw new Error(
        `Error fetching ingredient names: ${ingredientNamesError.message}`
      );
    }

    const ingredients = ingredientsData.map((ingredient) => {
      const ingredientData = ingredientNamesData.find(
        (nameData) => nameData.ingredientid === ingredient.ingredientid
      );
      return {
...
async function addUserAppliance(
  userId: string,
  ...
  try {
    ...
    if (!applianceId) {
      ...
      return new Response(
        JSON.stringify({
          error: `Failed to retrieve appliance ID for ${applianceName}`,
        }),
        {
          status: 400,
          headers: corsHeaders,
    ...
    if (userApplianceError) {
      ...
      return new Response(
        JSON.stringify({ error: userApplianceError.message }),
        {
          status: 400,
          headers: corsHeaders,
        }
...
async function removeUserAppliance(
  userId: string,
  applianceName: string,
  corsHeaders: HeadersInit
) {
...
async function getUserAppliances(userId: string, corsHeaders: HeadersInit) {
  try {
    ...
    const { data: userAppliancesData, error: userAppliancesError } =
      await supabase
        .from("userAppliances")
        .select("applianceid")
        .eq("userid", userId);
    ...
    if (userAppliancesError) {
      ...
      return new Response(
        JSON.stringify({ error: userAppliancesError.message }),
        {
          status: 400,
          headers: corsHeaders,
        }
    ...
    if (!userAppliancesData || userAppliancesData.length === 0) {
      return new Response(
        JSON.stringify({ error: "No appliances found for this user" }),
        {
          status: 404,
          headers: corsHeaders,
        }
    ...
    const applianceIds = userAppliancesData.map(
      (userAppliance) => userAppliance.applianceid
    );
    ...
    const userAppliances = userAppliancesData.map((userAppliance) => {
      const appliance = appliancesData.find(
        (appliance) => appliance.applianceid === userAppliance.applianceid
      );
      return {
...
async function addUserFavorite(
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "User favorite added successfully" }),
      {
        status: 200,
        headers: corsHeaders,
      }
...
async function removeUserFavorite(
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ message: "User favorite removed successfully" }),
      {
        status: 200,
        headers: corsHeaders,
      }
...
async function addRecipeKeywords(
  recipeid: string,
  keywords: string,
  corsHeaders: HeadersInit
) {
...
async function addRecipeDietaryConstraints(
  recipeid: string,
  dietaryConstraints: string,
  corsHeaders: HeadersInit
) {
  ...
  if (typeof dietaryConstraints !== "string") {
    ...
    return new Response(
      JSON.stringify({ error: "Invalid dietary constraints format" }),
      {
        status: 400,
        headers: corsHeaders,
      }
  ...
  if (dietaryConstraintError) {
    console.error(
      "Error updating recipe dietary constraints:",
      dietaryConstraintError
    );
    return new Response(
      JSON.stringify({ error: dietaryConstraintError.message }),
      {
        status: 400,
        headers: corsHeaders,
      }
  ...
  return new Response(
    JSON.stringify({
      success: "Recipe dietary constraints updated successfully",
    }),
    {
      status: 200,
      headers: corsHeaders,
