        ...
        setState(() {
          mealPlans = fetchedMealPlans.asMap().entries.map((entry) {
            ...
            final Map<String, dynamic> recipesData =
                jsonDecode(mealPlan['recipes']);
            final String name = recipesData['Name'] ?? 'Meal Plan ${index + 1}';
            final String description = recipesData['Description'] ?? '';

            final Map<String, dynamic> parsedRecipes =
                recipesData['Meals'] as Map<String, dynamic>;
      ...
      if (response.statusCode == 200) {
        setState(() {
          mealPlans.removeWhere(
              (mealPlan) => mealPlan['mealplannerid'] == mealPlanId);
        });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ? Center(
              child:
                  Lottie.asset('assets/planner_load.json'), // Loading spinner
            )
          : ListView(
                      ...
                      body: Column(
                        ...
                        children: [
                          if (mealPlan['description']
                              .isNotEmpty) // Only show if description is available
                            Padding(
