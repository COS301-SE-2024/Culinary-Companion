...
class _ShoppingListScreenState extends State<ShoppingListScreen> {
  ...
  Future<void> _addToShoppingList(String? userId, String ingredientName,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print(
            'Successfully added $ingredientName to shopping list with quantity $quantity $measurementUnit');
      } else {
        print(
  ...
  Future<void> _editShoppingListItem(String category, String item,
      double quantity, String measurementUnit) async {
    try {
      final response = await http.post(
        Uri.parse(
  ...
  Future<void> _removeFromShoppingList(String category, String item) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            'Failed to remove $ingredientName from shopping list: ${response.statusCode}');
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody['newQuantity'] != null) {
          print(
              'Updated quantity of $ingredientName in pantry list to ${responseBody['newQuantity']}');
        } else {
                        ...
                        Expanded(
                          child: _shoppingList.isEmpty
                              ...
                              : ListView(
                                  ...
                                  children:
                                      _shoppingList.entries.expand((entry) {
                                    return [
...
// Helper method to build a category header
  Widget _buildCategoryHeader(String title) {
    ...
    // Function to retrieve IconData
    IconData getCategoryIcon(String category) {
      return categoryIcons[category.toLowerCase()] ?? Icons.workspaces;
    }
                  ...
                  child: Form(
                    ...
                    child: Column(
                      children: <Widget>[
                        ...
                        Text(
                            'Measurement Unit: $_measurementUnit'), // Display the measurement unit
                      ],
                    ),
                  ),
                ),
                  ...
                  TextButton(
                    ...
                    onPressed: () {
                      if (formKey.currentState!.validate()) {
                        ...
                        _addItem(
                            category, selectedItem, quantity, _measurementUnit);
                        Navigator.of(context).pop();
                      }
                    },
                    child: const Text(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text('Measurement Unit: $measurementUnit',
                      style: TextStyle(
                          color: Colors.white)), // Display the measurement unit
                ],
              ),
            ),
          ),
